...
export default function createAPI ({ injectionKey, endpoints, ssr, state = {}, ...options }) {
    ...
    const api = {
        ...
        cancel (key) {
            return Promise.all(api.active.map(request => (!key || request.key === key) && request.cancel()))
        },
