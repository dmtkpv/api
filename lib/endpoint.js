...
export default function createEndpoint (key, api) {
    ...
    const endpoint = reactive({
        ...
        async quiet (...args) {
            ...
            const trigger = createTrigger(api, payload);
            return endpoint.promise = onFetch(config)
                .then(trigger.onFetch)
                .then(config => onRequest(config, payload))
    ...
    function onRequest (config, payload) {
        const request = api.axios(config);
        const set = response => payload.response = markRaw(response);
