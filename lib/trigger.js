...
export default function createTrigger (api, endpoint, config) {
    ...
    function onFetch (config) {
        ...
        if (!fn) return config;
        return call(fn, config).then(result => onFetch(result === undefined ? config : result)).catch(onError)
    }
    ...
    function onError (error) {
        ...
        if (!fn) throw error;
        return call(fn, config).then(result => result === undefined ? onError(error) : onSuccess(result)).catch(onError);
    }

    function onCancel (error) {
        const fn = getFn('onCancel');
        if (!fn) throw error;
        return call(fn, config).then(result => result === undefined ? onCancel(error) : onSuccess(result)).catch(onError);
    }
